...
import { ShieldCheck, Trophy, Users, Clock, ArrowRight, Sparkles, HeartHandshake, Award, CheckCircle2, PartyPopper, HardHat, Smartphone, FileText, ExternalLink, Radio } from "lucide-react";
...
export default function AboutPage() {
  return (
    <>
      ...
      {/* ─── SAFETY CERTIFICATIONS & PROPRIETARY TECHNOLOGY SECTION ─── */}
      <section className="py-20 md:py-24 bg-slate-50 border-t border-slate-200">
        <div className="container mx-auto px-4 md:px-6 max-w-7xl">
          <div className="text-center max-w-3xl mx-auto mb-14">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#0090c8]/10 border border-[#0090c8]/20 mb-4">
              <ShieldCheck className="w-4 h-4 text-[#0090c8]" />
              <span className="text-xs font-extrabold text-[#0090c8] tracking-[0.15em] uppercase">Enterprise Standards</span>
            </div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-slate-900 tracking-tight mb-4">
              OSHA Safety Compliance &amp; <span className="text-[#0090c8]">Smart Operations</span>
            </h2>
            <p className="text-base sm:text-lg text-slate-600 font-medium leading-relaxed">
              We take safety, communication, and accountability seriously. With certified supervisors and custom proprietary management software, we manage our cleaners so you never have to.
            </p>
          </div>

          <div className="grid lg:grid-cols-2 gap-8 items-stretch max-w-6xl mx-auto">
            {/* CARD 1: OSHA Safety & Compliance */}
            <div className="bg-white rounded-3xl p-8 sm:p-10 border border-slate-200 shadow-xl flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                ...
                <h3 className="text-2xl font-black text-slate-900 leading-snug">
                  Safety-First Protocols &amp; Certified Leadership
                </h3>

                <p className="text-slate-600 text-sm sm:text-base leading-relaxed font-medium">
                  Commercial cleaning, high-tech facilities, and post-construction projects demand strict safety standards. Enterprise Cleaning Corporation invests heavily in continuous OSHA safety education and certification for our field leadership.
                </p>

                <div className="space-y-3 pt-2">
                  {/* Supervisor Galo Cambi */}
                  <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">Project Supervisor</div>
                      <div className="text-base font-black text-slate-900">Galo Cambi</div>
                      <div className="text-xs font-semibold text-emerald-700 flex items-center gap-1.5 mt-0.5">
                        <CheckCircle2 className="w-3.5 h-3.5" /> OSHA 10-Hour General Industry Certified
                      </div>
                    </div>
                    <a
                      href="/certification/galo-osha.pdf"
                      target="_blank"
                    ...
                    >
                      <FileText className="w-3.5 h-3.5 text-[#FFE800]" />
                      <span>View Certificate</span>
                      <ExternalLink className="w-3 h-3 opacity-60" />
                    </a>
                  </div>

                  {/* Operations Lead Julio Biage */}
                  <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80">
                    <div className="text-xs font-bold text-slate-400 uppercase tracking-wider">Operations &amp; Safety Lead</div>
                    <div className="text-base font-black text-slate-900">Julio Biage</div>
                    <div className="text-xs font-semibold text-blue-700 flex items-center gap-1.5 mt-0.5">
                      <CheckCircle2 className="w-3.5 h-3.5" /> Advanced OSHA 30-Hour Safety Training (In Progress)
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center gap-2 text-xs font-semibold text-slate-500">
                ...
                <span>Zero Subcontractors • Full Background Checks • Comprehensive Liability &amp; Workers&apos; Comp</span>
              </div>
            </div>

            {/* CARD 2: Proprietary Software & Operational Groups */}
            <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-blue-950 text-white rounded-3xl p-8 sm:p-10 border border-slate-800 shadow-xl flex flex-col justify-between space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-[#0090c8]/20 text-[#00B8FF] flex items-center justify-center shrink-0 border border-[#0090c8]/30">
                    <Smartphone className="w-7 h-7" />
                  </div>
                  <span className="px-3 py-1 rounded-full bg-[#FFE800] text-slate-950 text-xs font-black uppercase tracking-wider shadow-sm">
                    Created in 2020
                  </span>
                </div>

                <div>
                  <div className="text-xs font-extrabold text-[#00B8FF] uppercase tracking-widest mb-1">
                    Custom Account Technology
                  </div>
                  <h3 className="text-2xl font-black text-white leading-snug">
                    &quot;We manage our cleaners, so you don&apos;t have to!&quot;
                  </h3>
                </div>

                <p className="text-slate-300 text-sm sm:text-base leading-relaxed font-medium">
                  Enterprise Cleaning Corporation built a dedicated proprietary software system in 2020 that tracks every facet of your building&apos;s maintenance in real time.
                </p>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  <div className="p-3.5 rounded-xl bg-white/5 border border-white/10">
                    <div className="text-xs font-bold text-[#FFE800] mb-0.5">GPS Geo-Fencing</div>
                    <div className="text-xs text-slate-300 font-medium leading-relaxed">
                      Real-time clock-in &amp; clock-out tracking verifies staff presence at your location.
                    </div>
                  </div>

                  <div className="p-3.5 rounded-xl bg-white/5 border border-white/10">
                    <div className="text-xs font-bold text-[#FFE800] mb-0.5">Direct Messaging</div>
                    <div className="text-xs text-slate-300 font-medium leading-relaxed">
                      Instant two-way notifications between on-site cleaners and operational supervisors.
                    </div>
                  </div>

                  <div className="p-3.5 rounded-xl bg-white/5 border border-white/10">
                    <div className="text-xs font-bold text-[#FFE800] mb-0.5">Digital Quality Grading</div>
                    <div className="text-xs text-slate-300 font-medium leading-relaxed">
                      Routine work evaluated on an ongoing basis to ensure consistently top standards.
                    </div>
                  </div>

                  <div className="p-3.5 rounded-xl bg-white/5 border border-white/10">
                    <div className="text-xs font-bold text-[#FFE800] mb-0.5">Multi-Crew Network</div>
                    <div className="text-xs text-slate-300 font-medium leading-relaxed">
                      Coordinated Task Force, Van Crew, Projects Team, and 24/7 Rapid Response.
                    </div>
                  </div>
                </div>
              </div>
